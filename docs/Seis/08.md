---
title: 地震定位
createTime: 2026/06/06 13:32:09
permalink: /Seis/3d4wdghk/
---

在地震定位中，震源 (hypocenter) 被看成一个点，震源在地表的垂直投影叫震中 (epicenter)。地震定位就是为了确定震源的物理位置 $(x,y,z)$ 以及发震时刻 $t$。

我们假设：
- 已知真实的地球速度模型
- 已知观测台站坐标以及到时数据

## 震中位置估算

设介质速度均匀，震源坐标为 $(x, y, z)$，观测台站坐标为 $(x_\alpha, y_\alpha, z_\alpha)$，发震时刻为 $t$，则理论到时为
$$
t_\alpha = \frac{1}{v} \sqrt{(x - x_\alpha)^2 + (y - y_\alpha)^2 + (z - z_\alpha)^2} + t
$$

以地表观测台站为坐标原点，台站与震中得连线为 $X$ 轴，则
$$t^P = \frac{1}{\alpha} \sqrt{x^2 + z^2} + t$$
$$t^S = \frac{1}{\beta} \sqrt{x^2 + z^2} + t$$

S 波与 P 波的到时差为
$$
t^S - t^P = (\frac{1}{\beta}-\frac{1}{\alpha}) \sqrt{x^2 + z^2}
$$

假设 $z/x \ll 1$，则有
$$
x \approx \frac{\alpha\beta}{\alpha-\beta} (t^S - t^P)
$$

上式表明如果能获得 S 波与 P 波的到时差，就能估算出震中距。如果有三个以上的台站，就可以粗略估计出震中位置。

## 孤立地震定位

定义震源模型参数矢量

$$\mathbf{m} = (t, \mathbf{r}) = (t, x, y, z)$$

若已知台站坐标，由确定的震源模型参数可以计算理论到时：

$$t(\mathbf{r}, t) = T(\mathbf{r}) + t$$

其中 $T(\mathbf{r})$ 表示 $\mathbf{r}$ 处激发的地震射线的理论走时。

设先验震源坐标为 $\mathbf{r}_0$，将走时 $T(\mathbf{r})$ 在 $\mathbf{r}_0$ 处做一阶泰勒展开，得
$$
T(\mathbf{r}) = T(\mathbf{r}_0) + \nabla T(\mathbf{r}_0) \cdot (\mathbf{r} - \mathbf{r}_0)
$$

将上式代入理论到时方程，得
$$
t(\mathbf{r}, t) = T(\mathbf{r}_0) + \nabla T(\mathbf{r}_0) \cdot (\mathbf{r} - \mathbf{r}_0) + t
$$

将理论到时替换为观测到时，得到方程组：
$$
t_\alpha = T_\alpha(\mathbf{r}_0) + \nabla T_\alpha(\mathbf{r}_0) \cdot (\mathbf{r} - \mathbf{r}_0) + t
$$

对应的矩阵形式为

$$
\begin{bmatrix}
t_1 - T_1(\mathbf{r}_0) \\
t_2 - T_2(\mathbf{r}_0) \\
\vdots \\
t_n - T_n(\mathbf{r}_0)
\end{bmatrix}
=
\begin{bmatrix}
\frac{\partial T_1(\mathbf{r}_0)}{\partial x} & \frac{\partial T_1(\mathbf{r}_0)}{\partial y} & \frac{\partial T_1(\mathbf{r}_0)}{\partial z} & 1 \\
\frac{\partial T_2(\mathbf{r}_0)}{\partial x} & \frac{\partial T_2(\mathbf{r}_0)}{\partial y} & \frac{\partial T_2(\mathbf{r}_0)}{\partial z} & 1 \\
\vdots & \vdots & \vdots & \vdots \\
\frac{\partial T_n(\mathbf{r}_0)}{\partial x} & \frac{\partial T_n(\mathbf{r}_0)}{\partial y} & \frac{\partial T_n(\mathbf{r}_0)}{\partial z} & 1 \\
\end{bmatrix}
\begin{bmatrix}
x - x_0 \\
y - y_0 \\
z - z_0 \\
t
\end{bmatrix}
$$

简写为

$$\mathbf{d} = \mathbf{G} \Delta\mathbf{m}$$

### 一维水平层状介质

参考光路可逆性，可以得到
$$
\nabla T(\mathbf{r}) \bigg|_{\mathbf{r}=\mathbf{r}_0} = - \frac{\mathbf{n}(\mathbf{r}_0)}{c}
$$

其中 $\mathbf{n}(\mathbf{r}_0)$ 为先验震源点 $\mathbf{r}_0$ 处射线的出射方向单位矢量，$c$ 为 $\mathbf{r}_0$ 处的介质速度。

![](/Seis/T-gradient.png)

于是有
$$
t_\alpha = T_\alpha(\mathbf{r}_0) - \frac{\mathbf{n}(\mathbf{r}_0)}{c} \cdot (\mathbf{r} - \mathbf{r}_0) + t
$$

![](/Seis/ray_direction_angle.png)

设第 $\alpha$ 条射线的出射角为 $\theta_\alpha$，第 $\alpha$ 个观测台相对于先验震中的方位角为 $\phi_\alpha$，则有
$$
\mathbf{n}(\mathbf{r}_0) = (\sin \theta_\alpha \cos \phi_\alpha, -\sin \theta_\alpha \cos \phi_\alpha, \pm \cos \theta_\alpha)
$$
$$
\frac{\mathbf{n}(\mathbf{r}_0)}{c} = (p_\alpha \sin\phi_\alpha, -p_\alpha \cos\phi_\alpha, \pm \sqrt{c^{-2} - p_\alpha^2})
$$
当射线出射方向向下时取正号，向上时取负号。

## 主地震相对定位

假设已知一个地震的震源参数，称其为主地震，确定主地震附近地震相对于主地震的位置的方法叫主地震相对定位。由于主震区以外介质的非均匀性影响对主震区所有地震的影响基本相同，所以相对定位可以消除主震区以外介质的影响，从而提高定位精度。

设主地震震源参数为 $\mathbf{m}_0 = (t_0, x_0, y_0, z_0)$，附近地震的震源参数为 $\mathbf{m} = (t, x, y, z)$。对附近地震到第 $i$ 个台站的到时在 $\mathbf{m}_0$ 处做一阶泰勒展开，得：
$$
t_i(\mathbf{m}) = t_i(\mathbf{m}_0) + \nabla T_i(\mathbf{m}_0) \cdot (\mathbf{m} - \mathbf{m}_0)
$$

将 $t_i(\mathbf{m})$ 替换为附近地震的观测到时 $t_i^{obs}$，$t_i(\mathbf{m}_0)$ 替换为主地震的观测到时 $t_{i,0}^{obs}$，$\nabla T_i$ 为已知主震处的走时偏导，即可得到关于相对位移 $\Delta \mathbf{m} = \mathbf{m} - \mathbf{m}_0$ 的线性方程：

$$
t_i^{obs} - t_{i,0}^{obs} = \nabla T_i(\mathbf{m}_0) \cdot \Delta \mathbf{m}
$$

通过联合多个台站的数据求解该方程组，可以得到附近小地震相对于主地震的发震时间差与位置差。这种方法极大地减小了从主震区到台站之间由于速度模型不准确带来的走时路径累积误差。

## 双差地震定位法

双差定位法（Waldhauser & Ellsworth, 2000）进一步发展了相对定位的思想，是目前非常主流的高精度定位方法。其核心假设是：如果两个地震（事件 $i$ 和事件 $j$）的空间距离远小于它们到某一台站 $k$ 的相对距离，则这两个地震到该台站的地震射线路径几乎相同，由于速度模型不准确带来的走时误差也近乎等同。

定义走时残差 $r$ 为观测到时与理论计算到时之差：
$$
r_i^k = (t_i^k)^{obs} - (t_i^k)^{cal}
$$

双差（Double-Difference）即为两个地震到同一个台站的走时残差之差：
$$
\Delta r_{ij}^k = r_i^k - r_j^k = \left( (t_i^k)^{obs} - (t_j^k)^{obs} \right) - \left( (t_i^k)^{cal} - (t_j^k)^{cal} \right)
$$

在一阶泰勒展开近似下，事件的走时残差可以表示为由于该事件震源参数调整量 $\Delta \mathbf{m}$ 引起的走时变化：
$$
\frac{\partial t_i^k}{\partial \mathbf{m}} \cdot \Delta \mathbf{m}_i - \frac{\partial t_j^k}{\partial \mathbf{m}} \cdot \Delta \mathbf{m}_j = \Delta r_{ij}^k
$$

将大规模地震序列中不同事件组合、多个台站对的数据联立，可构建大规模稀疏矩阵方程。通过共轭梯度法（LSQR）等算法求解出各事件相对位置的最佳调整量。双差法结合波形互相关（Cross-correlation）技术计算得到的极高精度观测到时差，可以获得毫秒级的到时精度，从而实现对断层精细结构的刻画。