...
当射线经过焦散点时，导数 $dX/dp$ 改变符号，直接代入会导致振幅出现虚数或相位跳变，射线理论在该点发散。为在高频渐近意义下恢复与波动理论的一致性，需引入 KMAH 数对相位进行系统修正。

定义 KMAH 数表示从震源到接收点的射线路径上穿越焦散面的次数。每次穿越焦散点，射线振幅需乘以相位因子 $e^{-i\pi/2}$，累积相位延迟为
$$\Delta \Phi_{\text{caustic}} = -\frac{\pi}{2} N_{\text{KMAH}}$$
修正后的射线理论位移场可统一写为
$$u(\mathbf{x}_r,\omega) \propto \frac{1}{\sqrt{|J(\mathbf{x}_r)|}} \, e^{i\omega T(\mathbf{x}_r)} \, e^{-i\frac{\pi}{2} N_{\text{KMAH}}} \cdot (\text{源项与界面系数})$$
其中 $J$ 为射线雅可比行列式（即前文 $ds$ 的广义形式），$T$ 为走时。
