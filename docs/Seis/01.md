...
在地震学中，为了简化分析，常引入勒让德变换定义截距时间 $\tau = T - p\Delta$。将上述 $T$ 和 $\Delta$ 的方程代入：
...
地球平面展开 (Earth Flattening Transformation, EFT) 通过对坐标和介质参数进行数学映射，将原本在球面坐标系中处理的方程，转换为在**笛卡尔坐标系**中处理的形式，同时保持走时关系的一致性。

在平面化过程中，我们需要将径向坐标 $r$ 映射为垂直深度 $z$，
