...
由于最终取实数解，因此可以将上式等价改写为

$$
u(x,t) = \int_{0}^{+\infty} \left[ A_k e^{i( \omega t - k x)} + B_k e^{i( \omega t + k x)} \right] dk
$$
...
$$
\phi(\mathbf{r},t) = \int_{\mathbf{k}} A_{\mathbf{k}} e^{i (\omega t - \mathbf{k} \cdot \mathbf{r})} d\mathbf{k}
$$
...
$$
\Psi(\mathbf{r},t) = \int_{\mathbf{k}} \mathbf{A}_{\mathbf{k}} e^{i (\omega t - \mathbf{k} \cdot \mathbf{r})} d\mathbf{k}
$$
